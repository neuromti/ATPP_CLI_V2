function plot_split_half(PWD,ROI,SUB_LIST,VOX_SIZE,MAX_CL_NUM)

sub=textread(SUB_LIST,'%s');
sub_num=length(sub);

file=strcat(PWD,'/validation_',num2str(sub_num),'_',num2str(VOX_SIZE),'mm/',ROI,'_index_split_half.mat');
v=load(file);
x=2:MAX_CL_NUM;
...
title(strcat(ROI,'. split half'),'FontSize',14);
set(gcf,'Color','w');

output=strcat(PWD,'/validation_',num2str(sub_num),'_',num2str(VOX_SIZE),'mm/',ROI,'_split_half.jpg');
...
title(strcat(ROI,'. split half VI'),'FontSize',14);
set(gcf,'Color','w');

output=strcat(PWD,'/validation_',num2str(sub_num),'_',num2str(VOX_SIZE),'mm/',ROI,'_split_half_vi.jpg');
export_fig(output,'-r300','-painters','-nocrop');
