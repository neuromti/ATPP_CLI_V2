...
class ConsensusClustering:
    def __init__(self, WD, ROI, SUBs, CL_NUM, GROUP_THRESH):
        self.WD = WD
        self.SUBs = SUBs
        self.CL_NUM = CL_NUM
        self.ROI = ROI
...
if __name__ == '__main__':
    ...
    WD, ROI, SUB, MAX_CL_NUM, GROUP_THRES = sys.argv[1:]

    with open(SUB, 'r') as f:
        SUB_LIST = [line.strip() for line in f]

    for CL_NUM in range(2, int(MAX_CL_NUM) + 1):
        print(f"[INFO] Starting Consensus Clustering for {ROI} with {CL_NUM} clusters")
        clustering = ConsensusClustering(WD, ROI, SUB_LIST, CL_NUM, GROUP_THRES)
        consensus_clustering = clustering.consensus_clustering_of_co_association_matrix()
        HEADERPATH = clustering.get_file_path_of_normalized_subjectspecific_clustering(SUB_LIST[0])
        HEADER = nib.load(HEADERPATH)
        nifti = nib.Nifti1Image(consensus_clustering, HEADER.affine)

        os.makedirs(f'{WD}/ConsensusClustering/{ROI}', exist_ok=True)
        nib.save(nifti, f'{WD}/ConsensusClustering/{ROI}/{ROI}_{CL_NUM}_clusters.nii.gz')
