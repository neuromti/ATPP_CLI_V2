...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --is_t1_in_diffusion) IS_T1_IN_DIFFUSION="$2"; shift ;;
    *) echo "Unknown argument: $1"; exit 1 ;;
  esac
  shift
done

IS_T1_IN_DIFFUSION=$(echo "${IS_T1_IN_DIFFUSION}" | tr '[:upper:]' '[:lower:]')
...
echo "Is T1 in Diffusion: ${IS_T1_IN_DIFFUSION}"
...
# Function to determine the extension of T1 image
get_t1_extension() {
    local t1_base="${WD}/${sub}/T1_${sub}"
    if [ -f "${t1_base}.nii.gz" ]; then
        echo "nii.gz"
    elif [ -f "${t1_base}.nii" ]; then
        echo "nii"
    else
        echo ""
    fi
...
check_and_add_mask() {
    ...
    if [ "$mask_type" = "TARGET_MASKS" ]; then
        if [ -n "${TARGET_MASKS}" ]; then
            if [ -f "${WD}/${sub}/target_paths.txt" ]; then
                mask_options+="--targetmasks=${WD}/${sub}/target_paths.txt --os2t "
            else
                echo "Warning: Target masks file ${WD}/${sub}/target_paths.txt not found."
            fi
        else
            echo "Warning: Target masks not specified. Falling back to --target2 and --omatrix2."

            if [[ "${IS_T1_IN_DIFFUSION}" == "1" || "${IS_T1_IN_DIFFUSION}" == "true" ]]; then
                T1_EXT=$(get_t1_extension)
                if [ -n "$T1_EXT" ]; then
                    t1_path="${WD}/${sub}/T1_${sub}.${T1_EXT}"
                else
                    echo "Error: T1_${sub}.nii(.gz) not found in ${WD}/${sub}. Skipping subject."
                    return 1
                fi
            else
                t1_path="${WD}/${sub}/T1_in_diffusion_space_${sub}.nii.gz"
                if [ ! -f "$t1_path" ]; then
                    echo "Error: ${t1_path} not found. Skipping subject."
                    return 1
                fi
            fi

            mask_options+="--target2=${t1_path} --omatrix2 "
        fi
    else
...
run_probtrackx() {
    ...
    check_and_add_mask "TARGET_MASKS" "${WD}/${sub}" "_targets" "--targetmasks" $TARGET_MASKS

    echo "${roi} probtrackx is running...!"
    $TRACK_COMMAND -m "${DATA_DIR}/${sub}/Diffusion.bedpostX/nodif_brain_mask" \
                   -o "${roi%.nii*}" \
                   -x "${WD}/${sub}/ROI_masks/${roi}" \
                   -l "${DIS_COR}" \
                   -c "${CUR_THRES}" \
                   -S "${N_STEPS}" \
                   --steplength="${LEN_STEP}" \
...
for sub in $(cat ${SUB_LIST}); do
    for roi in "${WD}/${sub}/ROI_masks/"*.nii*; do
        echo "Processing ${roi} for subject ${sub}"
        # Check if ROI file exists
        if [ -f "$roi" ]; then
            filename=$(basename -- "$roi")
            roi_base="${filename%.nii.gz}"
            # Run Probtrackx with the specified parameters
            run_probtrackx "$roi_base" "$sub" || {
                # Log the error if Probtrackx fails
                echo "Error processing ${roi_base} for subject ${sub}. Skipping..."
                echo "${sub}" >> "$WD/error_subjects.txt"
                continue
            }
        else
            echo "No ROI mask files found for subject ${sub} in ${WD}/${sub}/ROI_masks."
            echo "${sub}" >> "$WD/error_subjects.txt"
            continue
        fi
    done
