function [mpm_cluster]=cluster_mpm_validation(PWD,ROI,SUB,METHOD,VOX_SIZE,kc,MPM_THRES)
    ...
    vnii_ref=load_untouch_nii(strcat(PWD,'/',sub{1},'/',sub{1},'_',ROI,'_',METHOD,'/',num2str(VOX_SIZE),'mm/',num2str(VOX_SIZE),'mm_',ROI,'_',num2str(kc),'_Template_relabel_group.nii.gz'));
    ...
    for subi=1:sub_num
        sub_file=strcat(PWD,'/',sub{subi},'/',sub{subi},'_',ROI,'_',METHOD,'/',num2str(VOX_SIZE),'mm/',num2str(VOX_SIZE),'mm_',ROI,'_',num2str(kc),'_Template_relabel_group.nii.gz');
