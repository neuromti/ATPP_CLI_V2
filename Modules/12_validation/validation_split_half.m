function validation_split_half(PWD,ROI,SUB_LIST,METHOD,VOX_SIZE,MAX_CL_NUM,N_ITER,GROUP_THRES,MPM_THRES)
% split half strategy
...
MASK_FILE=strcat(PWD,'/group_',num2str(sub_num),'_',num2str(VOX_SIZE),'mm/',ROI,'_roimask_thr',num2str(GROUP_THRES),'.nii.gz');
...
for ti=1:N_ITER
    ...
    for kc=2:MAX_CL_NUM
        disp(['split_half: ',ROI,' kc=',num2str(kc),' ',num2str(ti),'/',num2str(N_ITER)]);

        mpm_cluster1=cluster_mpm_validation(PWD,ROI,list1_sub,METHOD,VOX_SIZE,kc,MPM_THRES);
        mpm_cluster2=cluster_mpm_validation(PWD,ROI,list2_sub,METHOD,VOX_SIZE,kc,MPM_THRES);
...
save(strcat(PWD,'/validation_',num2str(sub_num),'_',num2str(VOX_SIZE),'mm/',ROI,'_index_split_half.mat'),'dice','nminfo','cv','vi');

fp=fopen(strcat(PWD,'/validation_',num2str(sub_num),'_',num2str(VOX_SIZE),'mm/',ROI,'_index_split_half.txt'),'at');
if fp
    for kc=2:MAX_CL_NUM
